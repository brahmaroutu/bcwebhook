...
usage() {
    cat <<EOF
Generate certificate suitable for use with an cosi-defaultbc webhook service.

This script uses k8s' CertificateSigningRequest API to a generate a
certificate signed by k8s CA suitable for use with cosi-defaultbc webhook
...
[ -z "${service}" ] && service=cosi-defaultbc-webhook
[ -z "${secret}" ] && secret=cosi-defaultbc-webhook-certs
